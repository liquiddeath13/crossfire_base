...
struct Sig
{
    ...
    struct PackedCmp
    {
        ...
        static bool cmp(const Package<Entries...>&, void* pos)
        {
    ...
    struct SetCmp
    {
        ...
        static size_t cmp(const Set<Entries...>&, void* pos)
        {
            return SetComparator<Entries...>::equals(pos);
        }
    };
    ...
    struct CompoundCmp
    {
        ...
        static bool cmp(const Compound<Entries...>&, void* pos)
        {
            return Comparator<Entries...>::cmp(pos);
        }
    };
    ...
    struct Comparator<Entry, Entries...>
    {
        static bool cmp(void* pos)
        {
            ...
            {
                const bool matches = Comparator<Entry>::cmp(pos);
                return matches && Comparator<Entries...>::cmp(static_cast<unsigned char*>(pos) + Entry::k_size);
            }
            ...
            else if constexpr (Entry::k_tag == Tag::set)
            {
                ...
                return size && Comparator<Entries...>::cmp(static_cast<unsigned char*>(pos) + size);
            }
            ...
            {
                return Comparator<Entries...>::cmp(static_cast<unsigned char*>(pos) + Entry::k_size);
            }
        }
    };
    ...
    struct Comparator<Entry>
    {
        static bool cmp(void* pos)
        {
            ...
            {
                static_assert(xc("Invalid entry"));
            }
        }
    };
    ...
    template <typename... Entries>
    static void* find(void* buf, const size_t size)
    {
        constexpr auto k_patternSize = (Entries::k_size + ...);

        auto* pos = static_cast<unsigned char*>(buf);
        auto* end = static_cast<unsigned char*>(buf) + size - k_patternSize + 1;
        while (pos < end)
        {
    ...
    struct StrEq : Str<str>
    {
        static bool cmp(void* pos)
        {
            auto* mem = static_cast<const typename decltype(str)::Type*>(pos);
    ...
    struct StrEqNoCase : Str<str>
    {
        static bool cmp(void* pos)
        {
            ...
            auto* mem = static_cast<const Char*>(pos);
            for (size_t i = 0; i < decltype(str)::k_len; ++i)
            {
    ...
    struct MaskComparator<Entry, Entries...>
    {
        ...
        static bool cmp(const char data, const char pattern, const char subpattern, const char mask)
        {
            if (mask == Entry::k_char)
            {
                ...
                {
                    static_assert(xc("Unknown type of comparator"));
                }
            }
            else
            {
    ...
    static void* find(void* buf, const size_t size, const char* const sig, const char* const mask, const size_t sigsize)
    {
        if (!size || !sig || !mask)
        {
            return nullptr;
        ...
        auto* pos = static_cast<unsigned char*>(buf);
        auto* end = static_cast<unsigned char*>(buf) + size - sigsize + 1;
        while (pos < end)
        {
            bool result = true;
    ...
    // Pattern format: "\x11\x2\x00text" + "..?....", meaning of mask chars is customizable by Sig::Mask::* types
    template <typename... Comparators>
    static void* find(void* buf, const size_t size, const char* const sig, const char* const mask)
    {
        if (!mask)
        {
            return nullptr;
    ...
    template <typename... Comparators>
    static void* find(void* buf, const size_t size, const char* const sig, const char* const subsig, const char* const mask, const size_t sigsize)
    {
        if (!sig || !subsig || !mask)
        {
            return nullptr;
    ...
    // Pattern format: "\x11\x2\x00text" + "..?....", meaning of mask chars is customizable by Sig::Mask::* types
    template <typename... Comparators>
    static void* find(void* buf, const size_t size, const char* const sig, const char* const subsig, const char* const mask)
    {
        if (!mask)
        {
            return nullptr;
    ...
    static void* bitmask(void* buf, const size_t size, void* sig, void* mask, size_t sigsize)
    {
        if (!sig || !mask || !sigsize)
        {
            return nullptr;
        ...
        auto* val = static_cast<unsigned char*>(sig);
        auto* msk = static_cast<unsigned char*>(mask);
    ...
    static void* find(void* buf, const size_t size, const char* const sig)
    {
        if (!sig)
        {
            return nullptr;
        ...
        const auto testPattern = [&skipSpace, &calcTokenLen](void* buf, const char* const sig) -> bool
        {
            const char* sigPos = sig;
            unsigned char* bufPos = static_cast<unsigned char*>(buf);
        ...
        auto* pos = static_cast<unsigned char*>(buf);
        auto* end = static_cast<unsigned char*>(buf) + size - sigBytes + 1;
