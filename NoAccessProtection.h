...
//
//void test_func() {
//	printf(xc("HELLO FROM .text func!\n"));
//}
//
//#pragma optimize("", off) //Disable it so it doesn't get inlined
//#pragma section(".xkaya", execute, read, write) //Write so we can erase encryption func
//#pragma comment(linker,"/SECTION:.xkaya,ERW")
//#pragma code_seg(push, ".xkaya")
//
//uint8_t encryption_key;
//
//PIMAGE_SECTION_HEADER get_section_by_name(const char* name, HMODULE hMod) {
//	uint64_t modulebase = (uint64_t)hMod;
...
//}
//
//void encrypt_section(PIMAGE_SECTION_HEADER section, HMODULE hMod) {
//	uint64_t modulebase = (uint64_t)hMod;
...
//			printf(xc("Encrypted: %p\n"), address);
//		}
//		DWORD old;
//		vp((LPVOID)address, 0x1000, PAGE_EXECUTE_READWRITE, &old);
//		for (int off = 0; off < 0x1000; off += 0x1) {
//			*(BYTE*)(address + off) = _rotr8((*(BYTE*)(address + off) + 0x10) ^ encryption_key, 69);
//		}
//		vp((LPVOID)address, 0x1000, PAGE_NOACCESS, &old);
//	}
//}
//
...
//		vp((LPVOID)page_start, 0x1000, PAGE_READWRITE, &old);//Set write protection to decrypt
//		for (int off = 0; off < 0x1000; off += 0x1) {
//			*(BYTE*)(page_start + off) = (_rotl8(*(BYTE*)(page_start + off), 69) ^ encryption_key) - 0x10;
//		}
//		vp((LPVOID)page_start, 0x1000, PAGE_EXECUTE_READ, &old);//Set original protection
//		if (DebugConsole->IsAttached()) {
//			printf(xc("Decrypted %p rip %p\n"), page_start, ExceptionInfo->ContextRecord->Rip);
//		}
...
//}
//
//void InitializeNoAccessProtection(HMODULE hMod) {
//	srand(time(NULL));
...
//	encrypt_section(get_section_by_name(".text", hMod), hMod);
